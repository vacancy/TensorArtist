# -*- coding:utf8 -*-
# File   : data_provider_max_loss_explorer.py
# Author : Jiayuan Mao
#          Honghua Dong
...
class MyDataFlow(flow.SimpleDataFlowBase):
    def __init__(self, player, loss_func=None, output_next=False, shuffle=True, memory_size=10000):
        self._player = player
    ...
    def _get_data(self):
        ...
        if self._pred_func is None:
            ...
            losses = []
            for a in range(get_player_nr_actions()):
                feed_dict = {'state': [state], 'action': [a]}
                losses.append(self._loss_func(**feed_dict)[0])
            action = np.argmax(losses)
...
def make_dataflow_train(env):
    ...
    func = env.make_func()
    func.compile(env.g_loss)
    # from IPython import embed; embed()

    df_g = MyDataFlow(make_player(), loss_func=func, output_next=False)
...
def make_dataflow_demo(env):
    ...
    h, w, c = get_input_shape()
    df = MyDataFlow(make_player(), loss_func=func, shuffle=False, output_next=True)
