...
class Controller(object):
    def __init__(self):
        ...
        self._context.sndhwm = configs.CTL_CTL_HWM
        self._context.rcvhwm = configs.CTL_CTL_HWM
    ...
    def initialize(self, pipes=None):
        ...
        response = utils.req_send_and_recv(self._ns_socket, {
            'action': configs.Actions.NS_REGISTER_CTL_REQ,
            'uid': self._uid,
            'ctl_protocal': 'tcp',
            'ctl_addr': self._addr,
        ...
        assert response['action'] == configs.Actions.NS_REGISTER_CTL_REP

        # register pipes on name-server
        response = utils.req_send_and_recv(self._ns_socket, {
            'action': configs.Actions.NS_REGISTER_PIPE_REQ,
            'uid': self._uid,
            'ipipes': list(self._imanager.names),
            'opipes': list(self._omanager.names)
        })
        assert response['action'] == configs.Actions.NS_REGISTER_PIPE_REP

        # query name-server for ipipes
        response = utils.req_send_and_recv(self._ns_socket, {
            'action': configs.Actions.NS_QUERY_OPIPE_REQ,
            'ipipes': list(self._imanager.names)
        })
        ...
        # setup dispatcher
        self._control_dispatcher.register(configs.Actions.CTL_CONNECT_REP, self._on_ctl_connect_rep)
        self._control_dispatcher.register(configs.Actions.CTL_CONNECT_REQ, self._on_ctl_connect_req)
        self._control_dispatcher.register(configs.Actions.CTL_CONNECTED_REQ, self._on_ctl_connected_req)
        self._control_dispatcher.register(configs.Actions.CTL_NOTIFY_OPEN_REQ, self._on_ctl_notify_open_req)
        self._control_dispatcher.register(configs.Actions.CTL_NOTIFY_CLOSE_REQ, self._on_ctl_notify_close_req)
    ...
    def _main_heartbeat(self):
        while True:
            self._control_mqueue.put(ControlMessage(self._ns_socket, None, {
                'action': configs.Actions.NS_HEARTBEAT_REQ,
                'uid': self._uid
            }, countdown=0))
    ...
    def _do_setup_ctl_peer(self, uid):
        ...
        self._control_mqueue.put(ControlMessage(sock, None, {
            'action': configs.Actions.CTL_CONNECT_REQ,
            'uid': self._uid,
            'ipipes': list(self._imanager.names)
    ...
    def _on_ctl_connect_req(self, identifier, msg):
        ...
        for name in pipes:
            ...
            if port > 0:
                response[name] = {
                    'dat_protocal': configs.CTL_DAT_PROTOCAL,
                    'dat_addr': self._addr,
                    'dat_port': port
                }

        self._control_mqueue.put(ControlMessage(self._control_router, identifier, {
            'action': configs.Actions.CTL_CONNECT_REP,
            'uid': self._uid,
            'opipes': response
        }, countdown=configs.CTL_CTL_SND_COUNTDOWN))
